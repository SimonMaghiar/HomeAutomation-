...
void loop()  
{
 ...
 Serial.println(temp);
  ...
  if (!radio.write( &temp, sizeof(temp) )){  //if the write fails let the user know over serial monitor
     Serial.println("DHT_value delivery failed");      
  }
  else { //if the write was successful 
      Serial.println("Success sending DHT_value!");
      radio.powerDown();
  }  
  delay(50000);
}

//void NRF_transmit_value(struct Dht_Struct dht)
//{
//  
//  radio.openWritingPipe(PTXpipe);        //open writing or transmit pipe
//  if (!radio.write( &dht, sizeof(dht) )){  //if the write fails let the user know over serial monitor
//     Serial.println("DHT_value delivery failed");      
//  }
//  else { //if the write was successful 
//      Serial.println("Success sending DHT_value!");
