...
{
 radio.powerUp();
 int quality = analogRead(analog_pin);
 Serial.println(quality);
 NRF_transmit_value(quality);
 
 delay(60000);
}

void NRF_transmit_value(int MQ135_value)
{
  radio.openWritingPipe(PTXpipe);        //open writing or transmit pipe
  if (!radio.write( &MQ135_value, sizeof(MQ135_value) )){  //if the write fails let the user know over serial monitor
     Serial.println("MQ135_value delivery failed");      
  }
  else { //if the write was successful 
      Serial.print("Success sending MQ135_value!");
      radio.powerDown();
  }  
}
